...
for i = 1:nElem+1
    ...
    if dist < r0^2
        ft(i,:) = ForceComponent(k0, x0, y0, x(lnp(n(i), 1)), x(lnp(n(i), 2)));
    else
        ft(i, :) = [0, 0];
    end
...
y1 = -1.2; % Circle center posn Y
r1 = 1; % Circle radius
k1 = .05; %Spring constant
for i = 1:nElem+1
    dist = (x(lnp(n(i),1))-x1)^2+(x(lnp(n(i),2))-y1)^2;
    if dist < r1^2
        ft(i,:) = ForceComponent(k1, x1, y1, x(lnp(n(i), 1)), x(lnp(n(i), 2)));
    else
        ft(i, :) = [0, 0];
    end
...
r2 = 0.55; % Circle radius
k2 = .05; %Spring constant
for i = 1:nElem+1
    dist = (x(lnp(n(i),1))-x2)^2+(x(lnp(n(i),2))-y2)^2;
    if dist < r2^2
        ft(i,:) = ForceComponent(k2, x2, y2, x(lnp(n(i), 1)), x(lnp(n(i), 2)));
    else
        ft(i, :) = [0, 0];
    end
    f(2*i-1, 3) = f(2*i-1, 3) + ft(i, 1);
